...
#include "sobel_alg.h"
#include "NEON_2_SSE.h" /*neon header*/
using namespace cv;
...
void grayScale(Mat& img, Mat& img_gray_out)
{
  ...
  for (int i=0; i<img.rows; i++) {
	...
	for (int j=0; j<img.cols; j++) {
		step_ij = step_i + STEP1*j;
		
		// only load in one pixel at a time -- can do 4 pixels at a time at max cuz of register sizes neon registers are 128bit - represent in 8bits intervals?
		vld3.8 {q0,q1,q2}, [&img.data[step_ij]]

		r = // vld1q_f8(q0); switch to a store statement
		g = // vld1q_f8(q1);
		b = // vld1q_f8(q2);
	
		r = vmulq_n_f32(r, 0.114); // truncate decimal???
		g = vmulq_n_f32(g, 0.587);
		b = vmulq_n_f32(b, 0.299);
		
		r = vmulq_n_f32((vld1q_f32(d0), 0.114); // can combine like this??
			  
		r = vaddq_f32(r, g, b); // can u add 3 things at once?
	
		vst1_f32(img_gray_out.data[width_i + j], r); // store the vector back to memory
	}
