...
/* should be able to just delete */
	//should delete in order to have second thread
...
/* end of delete section */
  ...
  while (1) {
	...
	/* grayScale call */
    pc_start(&perf_counters);
		//if thread = 0 -> calculate gray tophalf
		
		//else thread = 1 -> calculate gray bottom half
		
    grayScale(src, img_gray);
		// add barrier to wait for both threads to finish grayscale
	...
	/* sobelCalc call */
    pc_start(&perf_counters);
		//if thread = 0 -> calculate sobel tophalf
		
		//else thread = 1 -> calculate sobel bottom half
