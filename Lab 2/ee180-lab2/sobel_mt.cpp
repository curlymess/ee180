...
void *runSobelMT(void *ptr)
{
  ...
  if(thread0_id == myID){
    ...
    // set img up for different threads
    if (thread0_id == myID) {
	adj_gray = img_gray(Range(0, (IMG_HEIGHT/2)+1), Range::all()); // top half
 	adj_sobel = img_sobel(Range(0, (IMG_HEIGHT/2)+1), Range::all());
    } else {
	adj_gray = img_gray(Range((IMG_HEIGHT/2)-1, IMG_HEIGHT), Range::all()); // bottom half
	adj_sobel = img_sobel(Range((IMG_HEIGHT/2)-1, IMG_HEIGHT), Range::all());
    }
