...
pthread_barrier_t endSobel, vid_barrier, q_barrier;
pthread_barrier_t gray_barrier, pregray_barrier, presobel_barrier, sobel_barrier, while_barrier;
...
int mainMultiThread()
{
  ...
  pthread_barrier_init(&q_barrier, NULL, 2);
  pthread_barrier_init(&vid_barrier, NULL, 2);

  pthread_barrier_init(&while_barrier, NULL, 2);
  pthread_barrier_init(&pregray_barrier, NULL, 2);
  pthread_barrier_init(&presobel_barrier, NULL, 2);
  pthread_barrier_init(&gray_barrier, NULL, 2);
  pthread_barrier_init(&sobel_barrier, NULL, 2); 
...
//pthread_barrier_wait(&endSobel); 
  ...
  pthread_barrier_destroy(&q_barrier);
  pthread_barrier_destroy(&vid_barrier);
  
  pthread_barrier_destroy(&while_barrier);
  pthread_barrier_destroy(&pregray_barrier);
  pthread_barrier_destroy(&presobel_barrier);
